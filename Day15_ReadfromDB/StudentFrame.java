...
public class StudentFrame extends javax.swing.JFrame
{
	...
	void Bshowdata_actionPerformed(java.awt.event.ActionEvent event)
	{
        ...
        dbconnect.Connect();
        ResultSet rs;
		dbconnect.DoUpdate("Insert into StudentInfo(StudentId,StudentName,Age,GPA) values('999999','Luk',30,3)");
		...
		try {
		  while (rs.next()) {
				...
				numGpa = Double.parseDouble(gpa);
				globalGPA += numGpa;
				count++;
                
                System.out.printf("ID is %s, Name is %s, age is %s and GPA is: %.2f\n",id,name,age,numGpa);
                
		    }
		} catch (SQLException ex) {
			System.err.println("error in database connection");
		}
		System.out.printf("\nAll GPA is %.2f",globalGPA/count); 
